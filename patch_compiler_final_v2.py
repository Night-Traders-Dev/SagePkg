...
# 1. Builtins list
content = content.replace('"clock", "input", "slice"', '"clock", "input", "exec", "readfile", "writefile", "getenv", "substr", "platform", "slice"')

# 2. emit_builtin_call handling
input_block = '''    if (strcmp(callee_name, "input") == 0) {
        if (call->arg_count == 0) {
...
content = content.replace(input_block, new_handling)

# 3. prelude prototypes AND implementations
# Insert after #include <string.h>

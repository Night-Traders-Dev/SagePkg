    ...
    for i in range(len(pkgs)):
        ...
        if files != nil:
            for j in range(len(files)):
                ...
                # io.exists isn't reliable for all systems, but we can try
                # Actually, let's just check if it's universal or arch-specific
                # We'll skip physical file check for now to avoid complexity with paths in CI
                let dummy = 0
